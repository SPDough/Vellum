...
const PythonREPLWorkflow: React.FC<PythonREPLWorkflowProps> = ({ open, onClose, config }) => {
  ...
  const { data: templates = {} } = useQuery({
    ...
    queryFn: () => api.get<any>('/data-workflows/templates'),
  });

  // Fetch REPL health status
  const { data: healthData, refetch: refetchHealth } = useQuery({
    queryKey: ['replHealth'],
    queryFn: () => api.get<any>('/data-workflows/repl/health'),
    enabled: open,
  ...
  const executeMutation = useMutation({
    mutationFn: (data: any) => api.post<any>('/data-workflows/execute', data),
    onSuccess: (result) => {
  ...
  const directReplMutation = useMutation({
    mutationFn: (data: any) => api.post<any>('/data-workflows/repl/execute', data),
  });
