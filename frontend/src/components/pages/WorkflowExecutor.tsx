...
const WorkflowExecutor: React.FC = () => {
  ...
  const loadWorkflowTemplates = async () => {
    try {
      const data = await api.get<WorkflowTemplate[]>('/workflow-execution/templates');
      setTemplates(data);
    } catch (err) {
  ...
  const loadActiveExecutions = async () => {
    try {
      const data = await api.get<WorkflowExecution[]>('/workflow-execution/executions');
      setExecutions(data);
    } catch (err) {
  ...
  const executeWorkflow = async () => {
    ...
    try {
      ...
      const result = await api.post<WorkflowExecution>('/workflow-execution/execute', {
  ...
  const executeTradeProcessing = async () => {
    ...
    try {
      ...
      const result = await api.post<WorkflowExecution>('/workflow-execution/execute-trade-processing', {
  ...
  const viewExecutionDetails = async (executionId: string) => {
    try {
      const execution = await api.get<WorkflowExecution>(`/workflow-execution/executions/${executionId}`);
