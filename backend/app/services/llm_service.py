        ...
        with tracer.start_as_current_span("openai_chat_completion") as span:
            ...
            try:
                response = await self.client.chat.completions.create(
                    ...
                    **{k: v for k, v in kwargs.items() if k in ["frequency_penalty", "presence_penalty", "stop"]}
                )
        ...
        **kwargs
        ...
        with tracer.start_as_current_span("anthropic_chat_completion") as span:
            ...
            try:
                ...
                span.set_attributes({
                    "llm.success": True,
                    "llm.tokens_used": response.usage.input_tokens + response.usage.output_tokens,
                    "llm.cost": cost
                })
        ...
        **kwargs
        ...
        with tracer.start_as_current_span("ollama_chat_completion") as span:
            ...
            try:
                ...
                estimated_tokens = self._estimate_tokens(messages, result.get("message", {}).get("content", ""))
            ...
            for name, fallback_provider in self.providers.items():
                if fallback_provider != chosen_provider:
                    try:
                        ...
                        return await fallback_provider.chat_completion(messages, **kwargs)
                    except Exception as fallback_error:
                        logger.warning(f"Fallback LLM provider {name} failed: {fallback_error}")
                        continue
