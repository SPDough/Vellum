...
class KnowledgeGraphSyncService:
                ...
                try:
                    ...
                    if existing:
                        await self.neo4j_service.update_entity(
                            EntityType.MCP_SERVER.value, server["id"], server_properties
                        )
                    else:
                        await self.neo4j_service.create_entity(
                            EntityType.MCP_SERVER.value, server_properties
                        )
                    ...
                    logger.error(
                        f"Failed to sync MCP server {server.get('id', 'unknown')}: {e}"
                    )

            logger.info(
                f"Successfully synced {synced_count} MCP servers to knowledge graph"
            )
            return synced_count
            ...
            for workflow in workflows:
                try:
                    ...
                    workflow_properties = {
                        ...
                        "trigger_schedule": workflow.get("execution_settings", {}).get(
                            "schedule"
                        ),
                    ...
                    if existing:
                        ...
                        await self.neo4j_service.create_entity(
                            EntityType.WORKFLOW.value, workflow_properties
                        )
                    ...
                    logger.error(
                        f"Failed to sync workflow {workflow.get('id', 'unknown')}: {e}"
                    )

            logger.info(
                f"Successfully synced {synced_count} workflows to knowledge graph"
            )
            return synced_count
    ...
    async def sync_data_streams_to_graph(
            ...
            for stream in data_streams:
                try:
                    ...
                    if existing:
                        ...
                        await self.neo4j_service.create_entity(
                            EntityType.DATA_STREAM.value, stream_properties
                        )
                    ...
                    logger.error(
                        f"Failed to sync data stream {stream.get('id', 'unknown')}: {e}"
                    )

            logger.info(
                f"Successfully synced {synced_count} data streams to knowledge graph"
            )
            return synced_count
    ...
    async def create_sample_financial_data(self):
        ...
        try:
            ...
            for security in securities:
                await self.neo4j_service.create_entity(
                    EntityType.SECURITY.value, security
                )
    ...
    async def sync_all_data(self):
        ...
        try:
            ...
            logger.info(
                f"Knowledge graph sync completed. Synced {mcp_count} MCP servers"
            )
