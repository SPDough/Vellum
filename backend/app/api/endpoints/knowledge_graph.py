...
from app.models.knowledge_graph import (
    BaseEntity, EntityType, RelationshipType, 
    KnowledgeGraphQuery, GraphTraversal, GraphAnalytics, GraphVisualization,
    Account, Security, Trade, Position, MCPServerEntity, WorkflowEntity, DataStreamEntity
)
...
async def get_graph_statistics(neo4j_service: Neo4jService = Depends(get_neo4j_service)):
...
async def create_entity(
    ...
    try:
        ...
        if 'id' not in request.properties:
            request.properties['id'] = f"{request.entity_type.value.lower()}_{uuid4().hex[:8]}"
...
async def execute_cypher_query(
    ...
    try:
        ...
        forbidden_keywords = ["DELETE", "DETACH", "REMOVE", "DROP", "CREATE INDEX", "CREATE CONSTRAINT"]
...
async def search_entities(
    ...
    try:
        ...
        if request.search_term:
            where_clauses.append("(n.name CONTAINS $search_term OR n.description CONTAINS $search_term)")
            ...
            for node_key in ['node', 'n', 'connected', 'm']:
                if node_key in record and record[node_key]:
                    ...
                    if node_id and node_id not in nodes:
                        label_key = f"{node_key}_labels" if f"{node_key}_labels" in record else "node_labels"
...
async def get_shortest_path(
    ...
    try:
        ...
        results = await neo4j_service.execute_cypher(query, {
