...
async def execute_data_workflow(
    ...
    try:
        ...
        config = await service.data_source_service.get_configuration(
            request.source_config_id
        )
        if not config:
            raise HTTPException(
                status_code=404, detail="Data source configuration not found"
            )
...
async def execute_sql_workflow(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to execute SQL workflow: {str(e)}"
        )
    ...
    try:
        ...
        for config_id in request.source_config_ids:
            ...
            if not config:
                raise HTTPException(
                    status_code=404,
                    detail=f"Data source configuration {config_id} not found",
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to execute pipeline workflow: {str(e)}"
        )
...
async def generate_automated_insights(
    ...
    try:
        ...
        result = await service.create_automated_insights_workflow(
            config=config, insight_type=request.insight_type
        )
    ...
    try:
        ...
        if len(prompt.strip()) < 10:
            ...
            validation_results["warnings"].append(
                "Prompt is too short. Provide more detailed instructions."
            )
        ...
        if not any(keyword in prompt.lower() for keyword in analysis_keywords):
            validation_results["suggestions"].append(
                "Consider adding specific action words like 'analyze', 'calculate', or 'identify'."
            )
        ...
        if "?" in prompt:
            validation_results["suggestions"].append(
                "Consider rephrasing questions as specific instructions for better results."
            )

        # Provide enhancement suggestions
        if validation_results["score"] > 80:
            validation_results["suggestions"].append(
                "Your prompt looks good! Consider adding specific output format requirements if needed."
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to validate prompt: {str(e)}"
        )
...
async def get_repl_capabilities(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get REPL capabilities: {str(e)}"
        )
...
async def execute_direct_repl(
    ...
    try:
        ...
        return {"success": True, "result": result, "execution_method": "direct_repl"}

    except Exception as e:
        return {"success": False, "error": str(e), "execution_method": "direct_repl"}
