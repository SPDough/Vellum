...
async def create_data_source(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create data source: {str(e)}"
        )
...
async def list_data_sources(
    ...
    data_source_type: Optional[DataSourceType] = Query(
...
async def get_data_source(
    config_id: str, service: DataSourceService = Depends(get_data_source_service)
) -> DataSourceConfigurationResponse:
...
async def update_data_source(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update data source: {str(e)}"
        )
...
async def delete_data_source(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete data source: {str(e)}"
        )
...
async def test_data_source(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to test data source: {str(e)}"
        )
...
async def execute_data_pull(
    ...
    try:
        return await service.execute_data_pull(
            config_id=config_id, trigger_type=trigger_type, triggered_by=triggered_by
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to execute data pull: {str(e)}"
        )
...
async def get_data_source_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get data source status: {str(e)}"
        )
...
async def toggle_data_source(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to toggle data source: {str(e)}"
        )
