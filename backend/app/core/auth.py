...
class KeycloakAuth:
    ...
    async def get_public_key(self) -> str:
        ...
        if (self._public_key_cache and 
            self._public_key_expires and 
            now < self._public_key_expires):
...
async def get_current_admin_user(current_user: User = Depends(get_current_user)) -> User:
    ...
    if "admin" not in current_user.roles and "otomeshon-admin" not in current_user.roles:
        raise HTTPException(
...
async def get_current_trader_user(current_user: User = Depends(get_current_user)) -> User:
