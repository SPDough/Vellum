...
these shared fixtures currently target `app.main_simple`, which means the
majority of tests using `client` / `async_client` are exercising the simplified
/demo backend rather than the canonical full backend in `app.main`.
"""
...
from app.main_simple import app as simple_app  # demo/dev-only backend under test
